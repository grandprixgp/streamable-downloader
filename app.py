...
parser.add_argument("--gdrive", metavar="Uploads to GDrive")
...
if args.gdrive:
    if os.name != "nt":
        ...
        print("Unfortunately, you are running on Windows, so invalid NTFS filename characters will be replaced with nothing. For example, a video with the title \"Are you for real?\" will be saved \
            with the filename: \"Are you for real.mp4")
...
if request.status_code == 200:
    ...
    ydl.download([video["json"]["url"] for video in data])
    [os.rename(video["location_on_disk"], videos_directory + "\\" + video["filename"]) for video in data]
